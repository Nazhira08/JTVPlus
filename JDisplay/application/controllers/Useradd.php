...
class Useradd extends MY_Controller {
  ...
  public function __construct() {
    ...
    if (cari_hak_akses($this->session->userdata('username'),'Useradd')=="0") redirect('dashboard');
    ...
    $this->data['id_menu']=  $this->db->select('id')->where("site",'Useradd')->get('menu')->result_array()[0]['id'];
    $this->data['id_menu_awal']=  $this->db->select('tree_menu')->where("site",'Useradd')->get('menu')->result_array()[0]['tree_menu'];
  } 
  ...
  public function index()
  {
    ...
    $this->data['konten'] = 'admin/useradd/index';
...
public function update() {
      ...
      if ($_POST) {
        //apakah data yang dirubah adalah Super user dengan asumsi id =2
        $apakah_su = $this->db->where('nip', $this->input->post('username'))->where('id','2')->from('menu_user')->count_all_results();
        if($apakah_su==0)
        {
            ...
            {
              $this->session->set_flashdata('alert', alert('Delete','Data gagal di hapus Karena User merupakan Super User'));
            }
         ...
         $this->data['konten'] = 'admin/useradd/tambah';
         $this->load->view('admin/layout/index', $this->data);
   ...
   public function tambah() {
      ...
      if ($_POST) 
      {
        ...
        else //user ada
        {
         ...
         $this->data['konten'] = 'admin/useradd/add';
         $this->load->view('admin/layout/index', $this->data);
         // print_r($this->data['query']);
   }
   ...
   public function delete() {
      ...
      if ($_POST) {
                //apakah data SU dengan asumsi Seting Su = 2
                $apakah_su = $this->db->where('nip', $this->input->post('username'))->where('id','2')->from('menu_user')->count_all_results();
                if($apakah_su==0)
                {
                  //cari foto
         ...
         $this->data['konten'] = 'admin/useradd/hapus';
         $this->load->view('admin/layout/index', $this->data);
         // print_r($this->data['query']);
      } else {
         $this->session->set_flashdata('alert', alert('error', status('404')));
