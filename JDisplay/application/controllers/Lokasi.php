...
class Lokasi extends MY_Controller {
  ...
  public function __construct() {
    ...
    if (cari_hak_akses($this->session->userdata('username'),'Lokasi')=="0") redirect('dashboard');
    ...
    $this->data['id_menu']=  $this->db->select('id')->where("site",'Lokasi')->get('menu')->result_array()[0]['id'];
    $this->data['id_menu_awal']=  $this->db->select('tree_menu')->where("site",'Lokasi')->get('menu')->result_array()[0]['tree_menu'];
  } 
  ...
  public function index()
  {
    ...
    $this->data['konten'] = 'admin/Lokasi/index';
...
public function tambah() {
      ...
      {
        //sleep(2);
        $ada_user = $this->db->where('lokasi', $this->input->post('lokasi'))->from('lokasi')->count_all_results();
        if($ada_user==0)
        {  
            if ($this->validation()) 
            {
                $this->db->insert('lokasi', $this->field_data()); 
            } 
            else 
            {
                    $this->session->set_flashdata('alert', alert('Simpan','Lokasi harus diisi'));
            }
        }
        else //user ada
        {
             $this->session->set_flashdata('alert', alert('Simpan', 'Lokasi Sudah ada'));
        }
         ...
         $this->data['konten'] = 'admin/Lokasi/add';
         $this->load->view('admin/layout/index', $this->data);
...
public function update() {
      ...
      if ($_POST) {
         ...
         $this->data['konten'] = 'admin/Lokasi/tambah';
         $this->load->view('admin/layout/index', $this->data);
         // print_r($this->data['query']);
      } else {
