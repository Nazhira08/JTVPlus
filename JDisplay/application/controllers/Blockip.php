...
class Blockip extends MY_Controller {
  ...
  public function __construct() {
    ...
    if (cari_hak_akses($this->session->userdata('username'),'Blockip')=="0") redirect('dashboard');
    ...
    $this->data['id_menu']=  $this->db->select('id')->where("site",'Blockip')->get('menu')->result_array()[0]['id'];
    $this->data['id_menu_awal']=  $this->db->select('tree_menu')->where("site",'Setinguser')->get('menu')->result_array()[0]['tree_menu'];
  } 
  ...
  public function index()
  {
    ...
    $this->data['konten'] = 'admin/blockip/index';
...
public function tambah() {
      ...
      {
        //sleep(2);
        $ada_user = $this->db->where('ip', $this->input->post('ip'))->where('open', '0')->from('ip_block')->count_all_results();
        if($ada_user==0)
        {  
            if ($this->validation()) 
            {
        ...
        {
             $this->session->set_flashdata('alert', alert('Simpan', 'Ip sudah ke block'));
        }
         ...
         $this->data['konten'] = 'admin/blockip/add';
         $this->load->view('admin/layout/index', $this->data);
...
public function update() {
      ...
      if ($_POST) {
         ...
         $this->data['konten'] = 'admin/blockip/tambah';
         $this->load->view('admin/layout/index', $this->data);
         // print_r($this->data['query']);
      } else {
