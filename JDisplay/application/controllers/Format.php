...
class Format extends MY_Controller {
  ...
  public function __construct() {
    ...
    if (cari_hak_akses($this->session->userdata('username'),'Format')=="0") redirect('dashboard');
    ...
    $this->data['id_menu']=  $this->db->select('id')->where("site",'Format')->get('menu')->result_array()[0]['id'];
    $this->data['id_menu_awal']=  $this->db->select('tree_menu')->where("site",'Format')->get('menu')->result_array()[0]['tree_menu'];
  } 
  ...
  public function index()
  {
    ...
    $this->data['judul'] = 'Format';
    $this->data['tombol'] = 'Cari';

    
    $this->data['alert'] = $this->session->flashdata('alert');
    $this->data['konten'] = 'admin/Format/index';
...
public function tambah() {
      ...
      {
        //sleep(2);
        $ada_user = $this->db->where('format', $this->input->post('format'))->from('format')->count_all_results();
        if($ada_user==0)
        {  
            if ($this->validation()) 
            {
                $this->db->insert('format', $this->field_data()); 
            } 
            else 
            {
                    $this->session->set_flashdata('alert', alert('Simpan','Format harus diisi'));
            }
        }
        else //user ada
        {
             $this->session->set_flashdata('alert', alert('Simpan', 'Format Sudah ada'));
        }
         ...
         $this->data['konten'] = 'admin/Format/add';
         $this->load->view('admin/layout/index', $this->data);
...
public function update() {
      ...
      if ($_POST) {
         ...
         $this->data['konten'] = 'admin/Format/tambah';
         $this->load->view('admin/layout/index', $this->data);
         // print_r($this->data['query']);
      } else {
