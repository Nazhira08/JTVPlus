...
if (!function_exists('cari')) {
   function cari($tanggal,$codeprg) {
      if (!is_valid_date($tanggal)) return NULL;
     $hasil=$this->data['query'] = $this->db
               ->select('sum(idlength) as jum_detik')
               ->where('tanggal',$tanggal )    
               ->where('code_program',$codeprg )   
               ->where('iso is not null' )   
               ->get('tmp_log' )->result_array()[0]['jum_detik'];
      return $hasil;
   }
}