...
class UploadHandler {
     ...
     */
    public function handleUpload($uploadDirectory,$No_berita, $name = null){
        ...
        # non-chunked upload
            ...
            if ($target){
                ...
                if (move_uploaded_file($file['tmp_name'], $target)){
                    //simpan di database jika berhasil
                    ...
                    $sql = "INSERT INTO so_file (id_so,path,uid, nama, besar, waktu_pengiriman,extension) VALUE ('$No_berita','$uploadDirectory','$uuid', '$name', '$size','".date('Y-m-d H:i:s')."','".$ext."')";
                    //$sql = "INSERT INTO so_file (id_so,uid) VALUE ('$No_berita','$uuid')";
    ...
    public function handleDelete($uploadDirectory, $name=null)
    {
        ...
        if (is_dir($target)){
            $this->removeDir($target);
             ...
             //delete di database jika berhasil
                    $servername = "localhost";
                    $database = "jtv_iklan";
                    $username = "jtv";
                    $password = "jtvku2017";
                    ...
                    // membuat koneksi
            $db = mysqli_connect($servername, $username, $password, $database);
            $query = mysql_query ("SELECT * FROM berita_file where uid='$uuid'");
            $jumlah= mysql_num_rows ($query); 
            if($jumlah==1) $sql = "delete from so_file where uid='$uuid'";
            else
            $sql = "delete from so_file where day(waktu_pengiriman)='".date('d')."' and month(waktu_pengiriman)='".date('m')."' and year(waktu_pengiriman)='".date('Y')."' and uid='$uuid'";
            $query = mysqli_query($db, $sql);
